...
int main(int argc,char *argv[])
{
   ...
   printf("process[%d]: processesCount = %d\n",rank,p);
   ...
   if(rank==1) {
		...
		printf("process[%d]: sent message %d to rank %d; sendTime = %d millisec\n", rank,x,dest,tSend);
   } else 
   if (rank==0) {
   		int y=0;
		MPI_Status status;
		...
		printf("process[%d]: received message %d from rank %d; recvTime = %d millisec\n",rank,y,status.MPI_SOURCE,tRecv);
   }
